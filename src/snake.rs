...
impl Snake {
    pub fn new() -> Self {
        Self {
            body: vec![Point { x: 2, y: 0 },Point { x: 1, y: 0 },Point { x: 0, y: 0 },],
    ...
    pub fn draw(&self) {
        ...
        for &Point { x, y } in self.body.iter() {
            wasm4::oval(x*8,y*8,8,8);
            //wasm4::rect(x * 8, y * 8, 8, 8);
        }

        set_draw_color(0x4);
        wasm4::oval(self.body[0].x * 8, self.body[0].y * 8, 8, 8);
    ...
    pub fn update(&mut self) -> Option<Point> {
        ...
        //wasm4::trace(self.body[0].x.to_string()) ;
        if self.body[0].x < 0 {
            self.body[0].x = 19;
        }
